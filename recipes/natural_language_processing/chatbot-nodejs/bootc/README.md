...
Details on the application can be found [in the chatbot-nodejs/README.md](../README.md). By default, this Containerfile includes a model-server
...
```bash
systemctl status chatbot-nodejs
```
...
```bash
podman kube play /usr/share/containers/systemd/chatbot-nodejs.yaml
```

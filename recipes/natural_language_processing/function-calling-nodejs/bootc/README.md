...
Details on the application can be found [in the function-calling-nodejs/README.md](../README.md). By default, this Containerfile includes a model-server
...
```bash
systemctl status function-calling-nodejs
```
...
```bash
systemctl stop function-calling-nodejs
```
...
```bash
podman kube play /usr/share/containers/systemd/function-calling-nodejs.yaml
```

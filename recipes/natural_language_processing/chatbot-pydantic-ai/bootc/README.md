...
Details on the application can be found [in the chatbot-pydantic-ai/README.md](../README.md). By default, this Containerfile includes a model-server
...
```bash
systemctl status chatbot-pydantic-ai
```
...
```bash
systemctl stop chatbot-pydantic-ai
```
...
```bash
podman kube play /usr/share/containers/systemd/chatbot-pydantic-ai.yaml
```
